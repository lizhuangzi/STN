...
class SRResirevBlock(nn.Module):
    def __init__(self, layercount=18,inchannel = 3,outchannel=64,first_initpad=False,pooling=True):
        ...
        for i in range(layercount):
            ...
            layer = nn.Sequential(nn.Conv2d(in_channels=inchannel, out_channels=inchannel, kernel_size=3, stride=1, padding=1, bias=False),
                                  nn.BatchNorm2d(outchannel),
                                  nn.ReLU(inplace=True),
                                  nn.Conv2d(in_channels=inchannel, out_channels=inchannel, kernel_size=3, stride=1,
...
class SRRIN(nn.Module):
    def __init__(self, nblock=[6,8,10,8],nblock2=[6,6,12,4],INi=[3,16,64,256]):
        ...
        self.resblock1 = SRResirevBlock(layercount=nblock[0],inchannel=16,outchannel=16,first_initpad=True)
        self.resblock2 = SRResirevBlock(layercount=nblock[1], inchannel=32,outchannel=32,first_initpad=True)

        self.resblock3 = SRResirevBlock(layercount=nblock[2], inchannel=64, outchannel=64,first_initpad=True,pooling=False)
        self.resblock4 = SRResirevBlock(layercount=nblock[2], inchannel=128, outchannel=128, first_initpad=True)


        self.bianda1 = nn.Sequential(
            nn.Conv2d(in_channels=16, out_channels=32, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
        )
        self.bianda2 = nn.Sequential(
            nn.Conv2d(in_channels=32, out_channels=64, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
        )
        self.bianda3 = nn.Sequential(
            nn.Conv2d(in_channels=64, out_channels=128, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
        )
        self.bianda4 = nn.Sequential(
        ...
        self.reconstruct = nn.Sequential(
            # nn.BatchNorm2d(256),48,31.414900467782168,0.8941112111399206,"tensor(1.00000e-05 *
